...
void processClientRequest(int connSock) {
	...
	char path[2048], buffer[2048], readbuffer[2048], readbuffer2[2048];
	char proc[5];
	char* cPath;
	char* dPath;
	...
	if( strcmp(proc, "GET" ) == 0){
		...
		readfd = read(fd, readbuffer2, 2048);
		if(readfd == - 1)
			//tell client error occured
			close(2);
			dup(connSock);
			...
			exit(EXIT_FAILURE);
                ...
                if (write(connSock, readbuffer2, strlen(readbuffer2)) < 0) {
                                perror("write");
                                exit(EXIT_FAILURE);
                                                                }
                        cout << "sent: " << readbuffer2;
